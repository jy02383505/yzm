...
<!-- head -->
	...
	<div class="panel panel-info">
		<div class="panel-heading"><h4>课程列表</h4></div>
		<div class="panel-body">
			<table class="table table-hover table-striped table-bordered">
				...
				<?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
						<td><?php echo ($i); ?></td>
						<td>
							<?php if($row[pid] == 0){ echo "<a href=".U("index", "id=$row[id]&path=$row[path]").">$row[name]</a>"; }else{ echo $row[name]; } ?>
						</td>
						<td>
							<?php if($row[pid] == 0){ echo "<a href=".U('add')."/id/$row[id]/path/$row[path]$row[id]".">添加子类</a>"; }else{ echo $row[path]; } ?>
						</td>
						<!-- <td><?php echo (substr($row[path],0,1)); ?></td> -->
						<?php if($row[type]==0){ $type="线下实训(首页展示)"; }elseif($row[type]==1){ $type="线上教育(首页展示)"; }elseif($row[type]==2){ $type="线下实训(列表展示)"; }else{ $type="线上教育(列表展示)"; } ?>
						<td><?php echo ($type); ?></td>
